      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-lg shadow-[#6366F1]/5 p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-[#475569]">Start your interview prep journey today</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <motion.div custom={4} variants={fieldVariants} initial="hidden" animate="visible">
              <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={isLoading} className="btn btn-primary w-full btn-lg disabled:opacity-50">
                {isLoading ? (
                  <div className="flex items-center gap-2"><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />Creating account...</div>
                ) : 'Create Account'}
              </motion.button>
            </motion.div>

            <motion.div custom={5} variants={fieldVariants} initial="hidden" animate="visible" className="flex items-center gap-3">
              <hr className="flex-1 border-[#E2E8F0]" /><span className="text-xs text-[#94A3B8]">OR</span><hr className="flex-1 border-[#E2E8F0]" />
            </motion.div>

            <motion.div custom={6} variants={fieldVariants} initial="hidden" animate="visible">
              <motion.button whileTap={{ scale: 0.97 }} type="button" className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white border-2 border-[#E2E8F0] rounded-xl text-[#475569] font-semibold hover:bg-[#F8FAFC] transition-colors cursor-pointer">
          ...
          <p className="text-center mt-6 text-sm text-[#475569]">
            Already have an account? <Link to="/login" className="text-[#6366F1] font-semibold hover:text-[#4F46E5] no-underline">Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
